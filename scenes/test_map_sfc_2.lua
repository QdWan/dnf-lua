...
local SceneMapSurfaceTest02 = class("SceneMapSurfaceTest02", SceneBase)
...
function SceneMapSurfaceTest02:init()
...
function SceneMapSurfaceTest02:setup_map()
...
function SceneMapSurfaceTest02:setupMapView()
...
function SceneMapSurfaceTest02:setupTileset()
...
function SceneMapSurfaceTest02:updateTilesetBatch()
    ...
    for x=0, max_x do
        for y=0, max_y do
            ...
            tilesetBatch:add(sprite.quad, x*tileSize, y*tileSize)
...
function SceneMapSurfaceTest02:moveMap(dx, dy, force_update)
...
function SceneMapSurfaceTest02:wheelmoved(x, y)
...
function SceneMapSurfaceTest02:draw()
    lg.draw(
        self.tilesetBatch,
...
function SceneMapSurfaceTest02:keypressed(key)
    if key == "kp-" then
        print("zooming out")
        self:set_zoom(-1)
    elseif key == "kp+" then
