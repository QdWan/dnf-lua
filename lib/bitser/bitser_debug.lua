...
serialize_value = function(value, seen)
    ...
    local valid_type = types[t]
    if valid_type then
        valid_type(value, seen)
    else
        local error_msg = "cannot serialize type " .. t .. tostring(value)
        if t == "function" then
            local debug_info = debug.getinfo(value)
            error_msg = error_msg .. "\n" .. inspect(debug_info)
        end
        error(error_msg)
    end
...
return {
    ...
    loadLoveFile = function(fname)
        ...
        Buffer_newDataReader(
            serializedData:getPointer(), serializedData:getSize())
